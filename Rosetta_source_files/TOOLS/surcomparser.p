...
PROGRAM surcomparser;
WITH surcomrules,surcomdecl,surcomgraphdef;
 procedure SURCOMPARSER_body(if1 :SURCOMDECL_pconnectionnode; 
                                            var if2 : SURCOMDECL_plostree);
{*earley constants:} 
...
type 
     psurfgraphnodelist = ^SURCOMDECL_surfgraphnodelist;
     plossurfnodelist = ^lossurfnodelist;
     lossurfnodelist = record
				next:plossurfnodelist;
				arg:SURCOMDECL_surfgraphnodelist;
		       end;
     ...
     setofstrees =  record
   			j:INTEGER;
			first:SURCOMDECL_plostree;
			last:SURCOMDECL_plostree;
		    end;
     ...
     rulerec = record
			...
			completed:BOOLEAN;
			nextnodes:SURCOMDECL_surfgraphnodelist;
			nexttops:SURCOMDECL_syntcatset;
			nextcats:SURCOMDECL_syntcatset
	       end;
     item = record
			cat:SURCOMDECL_syntcat;
     ...
     reltreelist1 = record
			first:SURCOMDECL_prsnode;
			last:SURCOMDECL_prsnode
		   end;
     sentencelength = 0..maximumlength;
     grammarlength = 0..SURCOMDECL_maxrnr;
...
var
    ...
    imax:sentencelength;
    topcats:SURCOMDECL_syntcatset;
    topcat:array[1..SURCOMDECL_maxrnr] of SURCOMDECL_syntcat;
    initialitem:array[1..SURCOMDECL_maxrnr] of pitem;
    libint:INTEGER;
    ...
    rsnodememory: array[1..maxrsnodes] of SURCOMDECL_rsnode;
    itemmemory: array[1..maxitems] of item;
    setofitemsmemory: array[1..maxsetofitems] 
							of setofitems;
    lostreememory: array[1..maxlostrees] of SURCOMDECL_lostree;
    ...
    lossurfnodememory: array[1..maxlossurfnodes] 
							of SURCOMDECL_lossurfgraphnode;
    lossurfnodelistmemory: array[1..maxlossurfnodelists] 
							of lossurfnodelist;
    absent1:SURCOMDECL_psurfgraph;
    newtop1:SURCOMDECL_psinglesegmentlist;
    Sij:setofstrees;
    parentlist:array[1..SURCOMDECL_maxrnr] of psetofitems;
...
procedure newrsnode(var p:SURCOMDECL_prsnode);
begin
new(p)
end;
...
procedure newlostree(var p:SURCOMDECL_plostree);
begin
new(p)
end;
...
procedure newlossurfnode(var p:SURCOMDECL_plossurfgraphnode);
begin
new(p)
end;

...
procedure taketree(var stree1:SURCOMDECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:SURCOMDECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1:SURCOMDECL_plostree;
...
procedure appendnode(node1:SURCOMDECL_psurfgraphnode;var S1:SURCOMDECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:SURCOMDECL_plossurfgraphnode;
...
procedure appendnodelist(S1:SURCOMDECL_surfgraphnodelist;var S2:SURCOMDECL_surfgraphnodelist);
...
procedure initnodelist(var S1:SURCOMDECL_surfgraphnodelist);
...
function stillnodes(var S1:SURCOMDECL_surfgraphnodelist):BOOLEAN;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:SURCOMDECL_psurfgraphnode;var S1:SURCOMDECL_surfgraphnodelist);
...
procedure copynodelist(var S1:SURCOMDECL_surfgraphnodelist;var S2:SURCOMDECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:SURCOMDECL_surfgraphnodelist;
...
procedure takenodelist(var S1:SURCOMDECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:SURCOMDECL_psnode;var RT1:reltreelist1);
var prsnode1:SURCOMDECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:SURCOMDECL_psurfgraph;var node1:SURCOMDECL_psurfgraphnode;
		var completed1:BOOLEAN;var nextnodes1:SURCOMDECL_surfgraphnodelist;
                var nexttops1:SURCOMDECL_syntcatset;var nextcats1:SURCOMDECL_syntcatset);
...
var nextnodes2:SURCOMDECL_surfgraphnodelist;
    result1,result2,result3,result4:SURCOMDECL_syntcatset;
    node2:SURCOMDECL_psurfgraphnode;
    cat1:SURCOMDECL_syntcat;
    completed2:BOOLEAN;
...
procedure searchorread(graph1:SURCOMDECL_psurfgraph;node2:SURCOMDECL_psurfgraphnode;
		var completed1:BOOLEAN;var nextnodes1:SURCOMDECL_surfgraphnodelist;
		var nexttops1:SURCOMDECL_syntcatset;var nextcats1:SURCOMDECL_syntcatset);
var node1:SURCOMDECL_psurfgraphnode;
begin
...
with item1^.ruleinfo do
	...
	searchorread(SURCOMDECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:INTEGER;node1:SURCOMDECL_psurfgraphnode;
...

function local(item1:pitem;stree1:SURCOMDECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:SURCOMDECL_surfgraphnodelist;
    node1:SURCOMDECL_psurfgraphnode;
    result:setofitems;
    rnr:INTEGER;

function localsurfrule(item1:pitem;stree1:SURCOMDECL_psnode;node1:SURCOMDECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:SURCOMDECL_nodeid;
    rnr,rnr2:INTEGER;
    parents1:setofitems;
    rnrset:array[0..SURCOMDECL_maxrnr] of BOOLEAN;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:SURCOMDECL_surfgraphnodelist;

begin
SURCOMDECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if SURCOMDECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
SURCOMrules_rule(rnr,n,stree1,loccond);
if SURCOMDECL_status then
	begin
	if SURCOMDECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>SURCOMDECL_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(SURCOMDECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if SURCOMDECL_debug then writeln('status false') end
end;

begin{local}
if SURCOMDECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
if stree1^.cat in item1^.ruleinfo.nextcats then
	...
	rnr:=0;
	while rnr<>SURCOMDECL_maxrnr do
		begin
		rnr:=rnr+1;
...
var 
    node1:SURCOMDECL_psurfgraphnode;
    surfgraphnodelist1:SURCOMDECL_surfgraphnodelist;
    result:setofstrees;
...
var prsnode1,prsnode2,prsnode3:SURCOMDECL_prsnode;
...
begin
if SURCOMDECL_debug then writeln('->finalsurfrule');
SURCOMDECL_assignstatus(false);
SURCOMDECL_assignnewtop(nil);
inittreelist(S1);
SURCOMrules_rule(rnr,SURCOMDECL_Hfinal,nil,globcond);
if SURCOMDECL_status then
	begin
	saversnodes(RT1);
	while SURCOMDECL_newtop<>nil do
		begin
                newtop1:=SURCOMDECL_newtop;
		...
		SURCOMDECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:SURCOMDECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:INTEGER);
var stree1:SURCOMDECL_psnode;
    node1:SURCOMDECL_psurfgraphnode;
begin
if SURCOMDECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if SURCOMDECL_status then
		begin
		SURCOMDECL_assignstatus(false);
		SURCOMrules_rule(rnr,node1^.n,stree1,globcond);
		nsons:=nsons+1;
...

begin{final}
if SURCOMDECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		SURCOMrules_rule(rnr,SURCOMDECL_Hinit,nil,globcond);
		nsons:=0;
...
if stilltrees(result)=false then if SURCOMDECL_debug then writeln('status false');
final:=result
end;

...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if SURCOMDECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
...
var list1:SURCOMDECL_psinglesegmentlist;
    i,j:INTEGER;
    stree1:SURCOMDECL_psnode;
    connode1,connode2:SURCOMDECL_pconnectionnode;
    Sij:setofstrees;
begin
...
i:=0;
if SURCOMDECL_debug then writeln('imax determined:',imax); 
while i<>imax do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if SURCOMDECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}if SURCOMDECL_debug then 
     		    begin
     		    if stree1^.cat=charstring then 
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if SURCOMDECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}if SURCOMDECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);SURCOMDECL_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>SURCOMDECL_maxrnr do
   begin
   i:=i+1;
...
if SURCOMDECL_debug then writeln('trees initialized');
initializeitems(imax);
if SURCOMDECL_debug then writeln('items initialized');
...
while i <> imax do
	begin
	if SURCOMDECL_debug then writeln(i);
