...
FUNCTION surflinkSCANNER_Body(VAR inf: text) : surflinkDECL_pconnectionnode; 
...
VAR
   ...
   result: surflinkDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN surflinkLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in surflinkLANGSPEC_scanchars);
  
         lower := 1; upper := surflinkLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; surflinkLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := surflinkLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := surflinkLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := surflinkLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := surflinkDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         surflinkDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   surflinkSCANNER_body := result


END {surflinkSCANNER_body};
