...
FUNCTION auxcomSCANNER_Body(VAR inf: text) : auxcomDECL_pconnectionnode; 
...
VAR
   ...
   t   : auxcomDECL_pSnode;
   result: auxcomDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN auxcomLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in auxcomLANGSPEC_scanchars);
  
         lower := 1; upper := auxcomLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; auxcomLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := auxcomLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := auxcomLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := auxcomLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := auxcomDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         auxcomDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   auxcomSCANNER_body := result


END {auxcomSCANNER_body};
