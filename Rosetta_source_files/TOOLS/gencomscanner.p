...
FUNCTION #SCANNER_Body(VAR inf: text) : #DECL_pconnectionnode; 
...
VAR
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN #LANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in #LANGSPEC_scanchars);
  
         lower := 1; upper := #LANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; #LANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := #LANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := #DECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         #DECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;
