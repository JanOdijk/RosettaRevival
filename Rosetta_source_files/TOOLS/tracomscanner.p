...
FUNCTION tracomSCANNER_Body(VAR inf: text) : tracomDECL_pconnectionnode; 
...
VAR
   ...
   t   : tracomDECL_pSnode;
   result: tracomDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN tracomLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in tracomLANGSPEC_scanchars);
  
         lower := 1; upper := tracomLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; tracomLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := tracomLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := tracomLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := tracomLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := tracomDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         tracomDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   tracomSCANNER_body := result


END {tracomSCANNER_body};
