...
FUNCTION mrucomSCANNER_Body(VAR inf: text) : mrucomDECL_pconnectionnode; 
...
VAR
   ...
   t   : mrucomDECL_pSnode;
   result: mrucomDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN mrucomLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in mrucomLANGSPEC_scanchars);
  
         lower := 1; upper := mrucomLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; mrucomLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := mrucomLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := mrucomLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := mrucomLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := mrucomDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         mrucomDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   mrucomSCANNER_body := result


END {mrucomSCANNER_body};
