...
VAR
   newtopvar   : tracomDECL_psinglesegmlist;
   statusvar   : boolean;
   debugvar    :  boolean;
   surfgraphvar: array[1..tracomRULES_MAXRNR] OF tracomDECL_psurfgraph;
   absentvar   : tracomDECL_psurfgraph;
   countervar  : INTEGER;
   warningvar  : BOOLEAN;
...
FUNCTION tracomDECL_newtop: tracomDECL_psinglesegmlist;
BEGIN
   tracomDECL_newtop:=newtopvar
END;

PROCEDURE tracomDECL_assignnewtop(valeu: tracomDECL_psinglesegmlist);
BEGIN
   newtopvar:=valeu
END;

PROCEDURE tracomDECL_addnewtop(a: tracomDECL_pSnode);
VAR
   c: tracomDECL_psinglesegmlist;
BEGIN
   new(c);
   WITH c^ DO
      BEGIN
...
FUNCTION tracomDECL_status:boolean;
BEGIN
   tracomDECL_status:=statusvar
END;

PROCEDURE tracomDECL_assignstatus(valeu:boolean);
BEGIN
   statusvar:=valeu
END;

FUNCTION tracomDECL_debug:boolean;
BEGIN
   tracomDECL_debug:=debugvar
END;

PROCEDURE tracomDECL_assigndebug(valeu:boolean);
BEGIN
   debugvar:=valeu
END;

FUNCTION tracomDECL_surfgraph(i:integer):tracomDECL_psurfgraph;
BEGIN
   tracomDECL_surfgraph:=surfgraphvar[i]
END;

PROCEDURE tracomDECL_assignsurfgraph(i:integer; valeu: tracomDECL_psurfgraph);
BEGIN
   surfgraphvar[i]:=valeu
END;

FUNCTION tracomDECL_absent: tracomDECL_psurfgraph;
BEGIN
   tracomDECL_absent:=absentvar
END;

PROCEDURE tracomDECL_assignabsent(valeu: tracomDECL_psurfgraph);
BEGIN
   absentvar:=valeu
END;

PROCEDURE tracomDECL_Addconnectionnode(VAR r: tracomDECL_pconnectionnode;
                                                t: tracomDECL_psnode); 
VAR p: tracomDECL_pconnectionnode;
    l: tracomDECL_psinglesegmlist;
...
FUNCTION tracomDECL_MakeTerminal(s: tracomLANGSPEC_Category;
                                          w: TSTRING_String): 
                                          tracomDECL_psnode;
VAR
   t : tracomDECL_psnode;
BEGIN
   ...
   t^.cat := s;
   t^.sonlist:=nil;
   tracomLANGSPEC_CreateTerm(s, w, t^.ldfield);
   tracomDECL_MakeTerminal := t
END;


FUNCTION tracomDECL_MakeNonTerminal(s: tracomLANGSPEC_Category):
                                          tracomDECL_psnode;
VAR
   t : tracomDECL_psnode;
BEGIN
   new(t);
   t^.cat := s;
   tracomDECL_MakeNonTerminal := t
END;

FUNCTION tracomDECL_Convinttostring(int:INTEGER):
                                          TSTRING_string;
...
tracomDECL_Convinttostring:=result
END;
