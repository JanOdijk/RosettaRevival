...
PROCEDURE surfcomPARSER_Body(if1 : surfcomDECL_pConnectionNode; var if2 : surfcomDECL_plostree);
{*earley constants:} 
...
type 
     psurfgraphnodelist = ^surfcomDECL_surfgraphnodelist;
     plossurfnodelist = ^lossurfnodelist;
     lossurfnodelist = record
				next:plossurfnodelist;
				arg:surfcomDECL_surfgraphnodelist;
		       end;
     ...
     setofstrees =  record
			j:integer;
			first:surfcomDECL_plostree;
			last:surfcomDECL_plostree;
		    end;
     ...
     rulerec = record
			...
			completed:boolean;
			nextnodes:surfcomDECL_surfgraphnodelist;
			nexttops:surfcomDECL_syntcatset;
			nextcats:surfcomDECL_syntcatset
	       end;
     item = record
			cat: surfcomLANGSPEC_Category;
     ...
     reltreelist1 = record
			first:surfcomDECL_prsnode;
			last:surfcomDECL_prsnode
		   end;
     sentencelength = 0..maximumlength;
     grammarlength = 0..surfcomRULES_maxrnr;
    
{*end earley types}

{*begin earley global variables:}
var la:surfcomDECL_psnode;
    i:integer;
    IT:array[0..maximumlength] of setofitems;
    ST:array[0..maximumlength] of setofsetofstrees;
    imax:sentencelength;
    topcats:surfcomDECL_syntcatset;
    initialitem:array[1..surfcomRULES_maxrnr] of pitem;
    libint:integer;
...
procedure newrsnode(var p:surfcomDECL_prsnode);
begin
   MEM_newdefault(loopholes.sizeof(surfcomDECL_rsnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlostree(var p:surfcomDECL_plostree);
begin
   MEM_newdefault(loopholes.sizeof(surfcomDECL_lostree),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlossurfnode(var p:surfcomDECL_plossurfgraphnode);
begin
   MEM_newdefault(loopholes.sizeof(surfcomDECL_lossurfgraphnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure taketree(var stree1: surfcomDECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:surfcomDECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1: surfcomDECL_plostree;
...
procedure appendnode(node1:surfcomDECL_psurfgraphnode;var S1:surfcomDECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:surfcomDECL_plossurfgraphnode;
...
procedure appendnodelist(S1:surfcomDECL_surfgraphnodelist;var S2:surfcomDECL_surfgraphnodelist);
...
procedure initnodelist(var S1:surfcomDECL_surfgraphnodelist);
...
function stillnodes(var S1:surfcomDECL_surfgraphnodelist):boolean;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:surfcomDECL_psurfgraphnode;var S1:surfcomDECL_surfgraphnodelist);
...
procedure copynodelist(var S1:surfcomDECL_surfgraphnodelist;var S2:surfcomDECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:surfcomDECL_surfgraphnodelist;
...
procedure takenodelist(var S1:surfcomDECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:surfcomDECL_psnode;var RT1:reltreelist1;
                        sonnumber:surfcomDECL_nodeid);
var prsnode1:surfcomDECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:surfcomDECL_psurfgraph;var node1:surfcomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:surfcomDECL_surfgraphnodelist;
                var nexttops1:surfcomDECL_syntcatset;var nextcats1:surfcomDECL_syntcatset);
...
var nextnodes2:surfcomDECL_surfgraphnodelist;
    result1,result2,result3,result4:surfcomDECL_syntcatset;
    node2:surfcomDECL_psurfgraphnode;
    cat1:surfcomLANGSPEC_Category;
    completed2:boolean;
...
procedure searchorread(graph1:surfcomDECL_psurfgraph;node2:surfcomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:surfcomDECL_surfgraphnodelist;
		var nexttops1:surfcomDECL_syntcatset;var nextcats1:surfcomDECL_syntcatset);
var node1:surfcomDECL_psurfgraphnode;
begin
...
with item1^ do
	begin
	cat:=surfcomGRAPHDEF_topcat(rnr1);
...
with item1^.ruleinfo do
	...
	searchorread(surfcomDECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:integer;node1:surfcomDECL_psurfgraphnode;
...
function local(item1:pitem;stree1:surfcomDECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:surfcomDECL_surfgraphnodelist;
    node1:surfcomDECL_psurfgraphnode;
    result:setofitems;
    rnr:integer;

function localsurfrule(item1:pitem;stree1:surfcomDECL_psnode;node1:surfcomDECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:surfcomDECL_nodeid;
    rnr,rnr2:integer;
    parents1:setofitems;
    rnrset:array[0..surfcomRULES_maxrnr] of boolean;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:surfcomDECL_surfgraphnodelist;

begin
surfcomDECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if surfcomDECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
surfcomRULES_rule(rnr,n,stree1,la,loccond);
if surfcomDECL_status then
	begin
	if surfcomDECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>surfcomRULES_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(surfcomDECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if surfcomDECL_debug then writeln('status false') end
end;

begin{local}
if surfcomDECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
if stree1^.cat in item1^.ruleinfo.nextcats then
	...
	rnr:=0;
	while rnr<>surfcomRULES_maxrnr do
		begin
		rnr:=rnr+1;
...
var 
    node1:surfcomDECL_psurfgraphnode;
    surfgraphnodelist1:surfcomDECL_surfgraphnodelist;
    result:setofstrees;
    stree1:surfcomDECL_psnode;
...
var S1:setofstrees;
    stree1,
    stree2 :surfcomDECL_psnode;
    Sij : setofstrees;

procedure saversnodes(var RT:reltreelist1);
var prsnode1,prsnode2,prsnode3:surfcomDECL_prsnode;

begin
prsnode1:=RT.first;
MEM_newdefault(loopholes.sizeof(surfcomDECL_rsnode),loopholes.retype(prsnode2,MEM_ptr));
prsnode2^:=prsnode1^;
RT.first:=prsnode2;
while prsnode1<>RT.last do
	...
	MEM_newdefault(loopholes.sizeof(surfcomDECL_rsnode),loopholes.retype(prsnode3,MEM_ptr));
        prsnode3^:=prsnode1^;
...
begin
if surfcomDECL_debug then writeln('->finalsurfrule');
surfcomDECL_assignstatus(false);
surfcomDECL_assignnewtop(nil);
inittreelist(S1);
readS(maxpos1, maxpos1+1, Sij);
taketree(stree2, Sij);
surfcomRULES_rule(rnr,Hfinal,nil,la,globcond);
if surfcomDECL_status then
	begin
	saversnodes(RT1);
	while surfcomDECL_newtop<>nil do
		begin
                newtop1:=surfcomDECL_newtop;
		...
		surfcomDECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:surfcomDECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:integer);
var stree1:surfcomDECL_psnode;
    node1:surfcomDECL_psurfgraphnode;
begin
if surfcomDECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if surfcomDECL_status then
		begin
		surfcomDECL_assignstatus(false);
		surfcomRULES_rule(rnr,node1^.n,stree1,la,globcond);
		nsons:=nsons+1;
...
begin{final}
if surfcomDECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		surfcomRULES_rule(rnr,Hinit,nil,la,globcond);
		nsons:=0;
		surfcomDECL_assignstatus(true);
		minpos:=item1^.minpos;
		maxpos:=item1^.curpos;
...
if stilltrees(result)=false then if surfcomDECL_debug then writeln('status false');
final:=result
end;
...
while rnr<>surfcomRULES_maxrnr do
	begin
	rnr:=rnr+1;
	if (surfcomGRAPHDEF_topcat(rnr) in item1^.ruleinfo.nexttops) then
		begin
...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if surfcomDECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
   ...
   {**}        IF i > (lasterror + 10) THEN
   			...
   			begin
                        ...
                        while w<>127 do
                           begin
                           w:=w+1;
                           IF surfcomLANGSPEC_scat(chr(w)) IN nextcats THEN
                              writeln('   ',chr(w))
                           end;
                        w:=0;
                        while w<>surfcomLANGSPEC_Nrow do
                           begin
                           w:=w+1;
                           IF surfcomLANGSPEC_wcat(w) IN nextcats THEN
                              BEGIN
                              surfcomLANGSPEC_word(w,str);
                              writeln('   ',str)
                              END
                           end;
...
var list1:surfcomDECL_psinglesegmlist;
    i,j:integer;
    stree1:surfcomDECL_psnode;
    connode1,connode2:surfcomDECL_pConnectionNode;
    Sij:setofstrees;
begin
...
if surfcomDECL_debug then writeln('imax determined:',imax); 
while i<>imax+1 do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if surfcomDECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}{ if surfcomDECL_debug then 
     		    begin
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if surfcomDECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}{ if surfcomDECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);surfcomdecl_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>surfcomRULES_maxrnr do
   begin
   i:=i+1;
...
{initialize topcats}
topcats := [];
FOR i:=1 TO surfcomRULES_maxrnr DO topcats := topcats + [surfcomGRAPHDEF_topcat(i)];
...
if surfcomDECL_debug then writeln('trees initialized');
initializeitems(imax);
if surfcomDECL_debug then writeln('items initialized');
...
while i <> imax do
	begin
	if surfcomDECL_debug then writeln(i);
