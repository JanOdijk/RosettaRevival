...
PROCEDURE mrucomPARSER_Body(if1 : mrucomDECL_pConnectionNode; var if2 : mrucomDECL_plostree);
{*earley constants:} 
...
type 
     psurfgraphnodelist = ^mrucomDECL_surfgraphnodelist;
     plossurfnodelist = ^lossurfnodelist;
     lossurfnodelist = record
				next:plossurfnodelist;
				arg:mrucomDECL_surfgraphnodelist;
		       end;
     ...
     setofstrees =  record
			j:integer;
			first:mrucomDECL_plostree;
			last:mrucomDECL_plostree;
		    end;
     ...
     rulerec = record
			...
			completed:boolean;
			nextnodes:mrucomDECL_surfgraphnodelist;
			nexttops:mrucomDECL_syntcatset;
			nextcats:mrucomDECL_syntcatset
	       end;
     item = record
			cat: mrucomLANGSPEC_Category;
     ...
     reltreelist1 = record
			first:mrucomDECL_prsnode;
			last:mrucomDECL_prsnode
		   end;
     sentencelength = 0..maximumlength;
     grammarlength = 0..mrucomRULES_maxrnr;
    
{*end earley types}

{*begin earley global variables:}
var la:mrucomDECL_psnode;
    i:integer;
    IT:array[0..maximumlength] of setofitems;
    ST:array[0..maximumlength] of setofsetofstrees;
    imax:sentencelength;
    topcats:mrucomDECL_syntcatset;
    initialitem:array[1..mrucomRULES_maxrnr] of pitem;
    libint:integer;
...
procedure newrsnode(var p:mrucomDECL_prsnode);
begin
   MEM_newdefault(loopholes.sizeof(mrucomDECL_rsnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlostree(var p:mrucomDECL_plostree);
begin
   MEM_newdefault(loopholes.sizeof(mrucomDECL_lostree),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlossurfnode(var p:mrucomDECL_plossurfgraphnode);
begin
   MEM_newdefault(loopholes.sizeof(mrucomDECL_lossurfgraphnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure taketree(var stree1: mrucomDECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:mrucomDECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1: mrucomDECL_plostree;
...
procedure appendnode(node1:mrucomDECL_psurfgraphnode;var S1:mrucomDECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:mrucomDECL_plossurfgraphnode;
...
procedure appendnodelist(S1:mrucomDECL_surfgraphnodelist;var S2:mrucomDECL_surfgraphnodelist);
...
procedure initnodelist(var S1:mrucomDECL_surfgraphnodelist);
...
function stillnodes(var S1:mrucomDECL_surfgraphnodelist):boolean;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:mrucomDECL_psurfgraphnode;var S1:mrucomDECL_surfgraphnodelist);
...
procedure copynodelist(var S1:mrucomDECL_surfgraphnodelist;var S2:mrucomDECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:mrucomDECL_surfgraphnodelist;
...
procedure takenodelist(var S1:mrucomDECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:mrucomDECL_psnode;var RT1:reltreelist1;
                        sonnumber:mrucomDECL_nodeid);
var prsnode1:mrucomDECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:mrucomDECL_psurfgraph;var node1:mrucomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:mrucomDECL_surfgraphnodelist;
                var nexttops1:mrucomDECL_syntcatset;var nextcats1:mrucomDECL_syntcatset);
...
var nextnodes2:mrucomDECL_surfgraphnodelist;
    result1,result2,result3,result4:mrucomDECL_syntcatset;
    node2:mrucomDECL_psurfgraphnode;
    cat1:mrucomLANGSPEC_Category;
    completed2:boolean;
...
procedure searchorread(graph1:mrucomDECL_psurfgraph;node2:mrucomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:mrucomDECL_surfgraphnodelist;
		var nexttops1:mrucomDECL_syntcatset;var nextcats1:mrucomDECL_syntcatset);
var node1:mrucomDECL_psurfgraphnode;
begin
...
with item1^ do
	begin
	cat:=mrucomGRAPHDEF_topcat(rnr1);
...
with item1^.ruleinfo do
	...
	searchorread(mrucomDECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:integer;node1:mrucomDECL_psurfgraphnode;
...
function local(item1:pitem;stree1:mrucomDECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:mrucomDECL_surfgraphnodelist;
    node1:mrucomDECL_psurfgraphnode;
    result:setofitems;
    rnr:integer;

function localsurfrule(item1:pitem;stree1:mrucomDECL_psnode;node1:mrucomDECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:mrucomDECL_nodeid;
    rnr,rnr2:integer;
    parents1:setofitems;
    rnrset:array[0..mrucomRULES_maxrnr] of boolean;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:mrucomDECL_surfgraphnodelist;

begin
mrucomDECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if mrucomDECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
mrucomRULES_rule(rnr,n,stree1,la,loccond);
if mrucomDECL_status then
	begin
	if mrucomDECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>mrucomRULES_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(mrucomDECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if mrucomDECL_debug then writeln('status false') end
end;

begin{local}
if mrucomDECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
if stree1^.cat in item1^.ruleinfo.nextcats then
	...
	rnr:=0;
	while rnr<>mrucomRULES_maxrnr do
		begin
		rnr:=rnr+1;
...
var 
    node1:mrucomDECL_psurfgraphnode;
    surfgraphnodelist1:mrucomDECL_surfgraphnodelist;
    result:setofstrees;
    stree1:mrucomDECL_psnode;
...
var S1:setofstrees;
    stree1,
    stree2 :mrucomDECL_psnode;
    Sij : setofstrees;

procedure saversnodes(var RT:reltreelist1);
var prsnode1,prsnode2,prsnode3:mrucomDECL_prsnode;

begin
prsnode1:=RT.first;
MEM_newdefault(loopholes.sizeof(mrucomDECL_rsnode),loopholes.retype(prsnode2,MEM_ptr));
prsnode2^:=prsnode1^;
RT.first:=prsnode2;
while prsnode1<>RT.last do
	...
	MEM_newdefault(loopholes.sizeof(mrucomDECL_rsnode),loopholes.retype(prsnode3,MEM_ptr));
        prsnode3^:=prsnode1^;
...
begin
if mrucomDECL_debug then writeln('->finalsurfrule');
mrucomDECL_assignstatus(false);
mrucomDECL_assignnewtop(nil);
inittreelist(S1);
readS(maxpos1, maxpos1+1, Sij);
taketree(stree2, Sij);
mrucomRULES_rule(rnr,Hfinal,nil,la,globcond);
if mrucomDECL_status then
	begin
	saversnodes(RT1);
	while mrucomDECL_newtop<>nil do
		begin
                newtop1:=mrucomDECL_newtop;
		...
		mrucomDECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:mrucomDECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:integer);
var stree1:mrucomDECL_psnode;
    node1:mrucomDECL_psurfgraphnode;
begin
if mrucomDECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if mrucomDECL_status then
		begin
		mrucomDECL_assignstatus(false);
		mrucomRULES_rule(rnr,node1^.n,stree1,la,globcond);
		nsons:=nsons+1;
...
begin{final}
if mrucomDECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		mrucomRULES_rule(rnr,Hinit,nil,la,globcond);
		nsons:=0;
		mrucomDECL_assignstatus(true);
		minpos:=item1^.minpos;
		maxpos:=item1^.curpos;
...
if stilltrees(result)=false then if mrucomDECL_debug then writeln('status false');
final:=result
end;
...
while rnr<>mrucomRULES_maxrnr do
	begin
	rnr:=rnr+1;
	if (mrucomGRAPHDEF_topcat(rnr) in item1^.ruleinfo.nexttops) then
		begin
...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if mrucomDECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
   ...
   {**}        IF i > (lasterror + 10) THEN
   			...
   			begin
                        ...
                        while w<>127 do
                           begin
                           w:=w+1;
                           IF mrucomLANGSPEC_scat(chr(w)) IN nextcats THEN
                              writeln('   ',chr(w))
                           end;
                        w:=0;
                        while w<>mrucomLANGSPEC_Nrow do
                           begin
                           w:=w+1;
                           IF mrucomLANGSPEC_wcat(w) IN nextcats THEN
                              BEGIN
                              mrucomLANGSPEC_word(w,str);
                              writeln('   ',str)
                              END
                           end;
...
var list1:mrucomDECL_psinglesegmlist;
    i,j:integer;
    stree1:mrucomDECL_psnode;
    connode1,connode2:mrucomDECL_pConnectionNode;
    Sij:setofstrees;
begin
...
if mrucomDECL_debug then writeln('imax determined:',imax); 
while i<>imax+1 do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if mrucomDECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}{ if mrucomDECL_debug then 
     		    begin
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if mrucomDECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}{ if mrucomDECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);mrucomdecl_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>mrucomRULES_maxrnr do
   begin
   i:=i+1;
...
{initialize topcats}
topcats := [];
FOR i:=1 TO mrucomRULES_maxrnr DO topcats := topcats + [mrucomGRAPHDEF_topcat(i)];
...
if mrucomDECL_debug then writeln('trees initialized');
initializeitems(imax);
if mrucomDECL_debug then writeln('items initialized');
...
while i <> imax do
	begin
	if mrucomDECL_debug then writeln(i);
