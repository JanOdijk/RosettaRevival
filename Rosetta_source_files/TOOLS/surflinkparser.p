...
PROCEDURE surflinkPARSER_Body(if1 : surflinkDECL_pConnectionNode; var if2 : surflinkDECL_plostree);
{*earley constants:} 
...
type 
     psurfgraphnodelist = ^surflinkDECL_surfgraphnodelist;
     plossurfnodelist = ^lossurfnodelist;
     lossurfnodelist = record
				next:plossurfnodelist;
				arg:surflinkDECL_surfgraphnodelist;
		       end;
     ...
     setofstrees =  record
			j:integer;
			first:surflinkDECL_plostree;
			last:surflinkDECL_plostree;
		    end;
     ...
     rulerec = record
			...
			completed:boolean;
			nextnodes:surflinkDECL_surfgraphnodelist;
			nexttops:surflinkDECL_syntcatset;
			nextcats:surflinkDECL_syntcatset
	       end;
     item = record
			cat: surflinkLANGSPEC_Category;
     ...
     reltreelist1 = record
			first:surflinkDECL_prsnode;
			last:surflinkDECL_prsnode
		   end;
     sentencelength = 0..maximumlength;
     grammarlength = 0..surflinkRULES_maxrnr;
    
{*end earley types}

{*begin earley global variables:}
var la:surflinkDECL_psnode;
    i:integer;
    IT:array[0..maximumlength] of setofitems;
    ST:array[0..maximumlength] of setofsetofstrees;
    imax:sentencelength;
    topcats:surflinkDECL_syntcatset;
    initialitem:array[1..surflinkRULES_maxrnr] of pitem;
    libint:integer;
    ...
    rsnodememory: array[1..maxrsnodes] of surflinkDECL_rsnode;
    itemmemory: array[1..maxitems] of item;
    setofitemsmemory: array[1..maxsetofitems] of setofitems;
    lostreememory: array[1..maxlostrees] of surflinkDECL_lostree;
    lossetofstreesmemory: array[1..maxlossetofstrees] of lossetofstrees;
    lositemmemory: array[1..maxlositems] of lositem;
    lossurfnodememory: array[1..maxlossurfnodes] of surflinkDECL_lossurfgraphnode;
    lossurfnodelistmemory: array[1..maxlossurfnodelists] of lossurfnodelist;
    absent1:surflinkDECL_psurfgraph;
    newtop1:surflinkDECL_psinglesegmlist;
    Sij:setofstrees;
    parentlist:array[1..surflinkRULES_maxrnr] of psetofitems;
    usedparentlistindices:set of grammarlength;
 {*} lasterror: integer;
{*einde earley global variables}
  
   stree2static:surflinkDECL_psnode;
...
procedure newrsnode(var p:surflinkDECL_prsnode);
begin
   MEM_newdefault(loopholes.sizeof(surflinkDECL_rsnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlostree(var p:surflinkDECL_plostree);
begin
   MEM_newdefault(loopholes.sizeof(surflinkDECL_lostree),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlossurfnode(var p:surflinkDECL_plossurfgraphnode);
begin
   MEM_newdefault(loopholes.sizeof(surflinkDECL_lossurfgraphnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure taketree(var stree1: surflinkDECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:surflinkDECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1: surflinkDECL_plostree;
...
procedure appendnode(node1:surflinkDECL_psurfgraphnode;var S1:surflinkDECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:surflinkDECL_plossurfgraphnode;
...
procedure appendnodelist(S1:surflinkDECL_surfgraphnodelist;var S2:surflinkDECL_surfgraphnodelist);
...
procedure initnodelist(var S1:surflinkDECL_surfgraphnodelist);
...
function stillnodes(var S1:surflinkDECL_surfgraphnodelist):boolean;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:surflinkDECL_psurfgraphnode;var S1:surflinkDECL_surfgraphnodelist);
...
procedure copynodelist(var S1:surflinkDECL_surfgraphnodelist;var S2:surflinkDECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:surflinkDECL_surfgraphnodelist;
...
procedure takenodelist(var S1:surflinkDECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:surflinkDECL_psnode;var RT1:reltreelist1;
                        sonnumber:surflinkDECL_nodeid);
var prsnode1:surflinkDECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:surflinkDECL_psurfgraph;var node1:surflinkDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:surflinkDECL_surfgraphnodelist;
                var nexttops1:surflinkDECL_syntcatset;var nextcats1:surflinkDECL_syntcatset);
...
var nextnodes2:surflinkDECL_surfgraphnodelist;
    result1,result2,result3,result4:surflinkDECL_syntcatset;
    node2:surflinkDECL_psurfgraphnode;
    cat1:surflinkLANGSPEC_Category;
    completed2:boolean;
...
procedure searchorread(graph1:surflinkDECL_psurfgraph;node2:surflinkDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:surflinkDECL_surfgraphnodelist;
		var nexttops1:surflinkDECL_syntcatset;var nextcats1:surflinkDECL_syntcatset);
var node1:surflinkDECL_psurfgraphnode;
begin
...
with item1^ do
	begin
	cat:=surflinkGRAPHDEF_topcat(rnr1);
...
with item1^.ruleinfo do
	...
	searchorread(surflinkDECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:integer;node1:surflinkDECL_psurfgraphnode;
...
function local(item1:pitem;stree1:surflinkDECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:surflinkDECL_surfgraphnodelist;
    node1:surflinkDECL_psurfgraphnode;
    result:setofitems;
    rnr:integer;

function localsurfrule(item1:pitem;stree1:surflinkDECL_psnode;node1:surflinkDECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:surflinkDECL_nodeid;
    rnr,rnr2:integer;
    parents1:setofitems;
    rnrset:array[0..surflinkRULES_maxrnr] of boolean;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:surflinkDECL_surfgraphnodelist;

begin
surflinkDECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if surflinkDECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
surflinkRULES_rule(rnr,n,stree1,la,loccond);
if surflinkDECL_status then
	begin
	if surflinkDECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>surflinkRULES_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(surflinkDECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if surflinkDECL_debug then writeln('status false') end
end;

begin{local}
if surflinkDECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
if stree1^.cat in item1^.ruleinfo.nextcats then
	...
	rnr:=0;
	while rnr<>surflinkRULES_maxrnr do
		begin
		rnr:=rnr+1;
...
var 
    node1:surflinkDECL_psurfgraphnode;
    surfgraphnodelist1:surflinkDECL_surfgraphnodelist;
    result:setofstrees;
    stree1:surflinkDECL_psnode;
...
var S1:setofstrees;
    stree1,
    stree2 :surflinkDECL_psnode;
    Sij : setofstrees;

procedure saversnodes(var RT:reltreelist1);
var prsnode1,prsnode2,prsnode3:surflinkDECL_prsnode;

begin
prsnode1:=RT.first;
MEM_newdefault(loopholes.sizeof(surflinkDECL_rsnode),loopholes.retype(prsnode2,MEM_ptr));
prsnode2^:=prsnode1^;
RT.first:=prsnode2;
while prsnode1<>RT.last do
	...
	MEM_newdefault(loopholes.sizeof(surflinkDECL_rsnode),loopholes.retype(prsnode3,MEM_ptr));
        prsnode3^:=prsnode1^;
...
begin
if surflinkDECL_debug then writeln('->finalsurfrule');
surflinkDECL_assignstatus(false);
surflinkDECL_assignnewtop(nil);
inittreelist(S1);
readS(maxpos1, maxpos1+1, Sij);
taketree(stree2, Sij);
surflinkRULES_rule(rnr,Hfinal,nil,la,globcond);
if surflinkDECL_status then
	begin
	saversnodes(RT1);
	while surflinkDECL_newtop<>nil do
		begin
                newtop1:=surflinkDECL_newtop;
		...
		surflinkDECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:surflinkDECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:integer);
var stree1:surflinkDECL_psnode;
    node1:surflinkDECL_psurfgraphnode;
begin
if surflinkDECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if surflinkDECL_status then
		begin
		surflinkDECL_assignstatus(false);
		surflinkRULES_rule(rnr,node1^.n,stree1,la,globcond);
		nsons:=nsons+1;
...
begin{final}
if surflinkDECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		surflinkRULES_rule(rnr,Hinit,nil,la,globcond);
		nsons:=0;
		surflinkDECL_assignstatus(true);
		minpos:=item1^.minpos;
		maxpos:=item1^.curpos;
...
if stilltrees(result)=false then if surflinkDECL_debug then writeln('status false');
final:=result
end;
...
while rnr<>surflinkRULES_maxrnr do
	begin
	rnr:=rnr+1;
	if (surflinkGRAPHDEF_topcat(rnr) in item1^.ruleinfo.nexttops) then
		begin
...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if surflinkDECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
   ...
   {**}        IF i > (lasterror + 10) THEN
   			...
   			begin
                        ...
                        while w<>127 do
                           begin
                           w:=w+1;
                           IF surflinkLANGSPEC_scat(chr(w)) IN nextcats THEN
                              writeln('   ',chr(w))
                           end;
                        w:=0;
                        while w<>surflinkLANGSPEC_Nrow do
                           begin
                           w:=w+1;
                           IF surflinkLANGSPEC_wcat(w) IN nextcats THEN
                              BEGIN
                              surflinkLANGSPEC_word(w,str);
                              writeln('   ',str)
                              END
                           end;
...
var list1:surflinkDECL_psinglesegmlist;
    i,j:integer;
    stree1:surflinkDECL_psnode;
    connode1,connode2:surflinkDECL_pConnectionNode;
    Sij:setofstrees;
begin
...
if surflinkDECL_debug then writeln('imax determined:',imax); 
while i<>imax+1 do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if surflinkDECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}{ if surflinkDECL_debug then 
     		    begin
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if surflinkDECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}{ if surflinkDECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);surflinkdecl_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>surflinkRULES_maxrnr do
   begin
   i:=i+1;
...
{initialize topcats}
topcats := [];
FOR i:=1 TO surflinkRULES_maxrnr DO topcats := topcats + [surflinkGRAPHDEF_topcat(i)];
...
if surflinkDECL_debug then writeln('trees initialized');
initializeitems(imax);
if surflinkDECL_debug then writeln('items initialized');
...
while i <> imax do
	begin
	if surflinkDECL_debug then writeln(i);
