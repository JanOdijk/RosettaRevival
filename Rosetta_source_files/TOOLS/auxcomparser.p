...
PROCEDURE auxcomPARSER_Body(if1 : auxcomDECL_pConnectionNode; var if2 : auxcomDECL_plostree);
{*earley constants:} 
...
type 
     psurfgraphnodelist = ^auxcomDECL_surfgraphnodelist;
     plossurfnodelist = ^lossurfnodelist;
     lossurfnodelist = record
				next:plossurfnodelist;
				arg:auxcomDECL_surfgraphnodelist;
		       end;
     ...
     setofstrees =  record
			j:integer;
			first:auxcomDECL_plostree;
			last:auxcomDECL_plostree;
		    end;
     ...
     rulerec = record
			...
			completed:boolean;
			nextnodes:auxcomDECL_surfgraphnodelist;
			nexttops:auxcomDECL_syntcatset;
			nextcats:auxcomDECL_syntcatset
	       end;
     item = record
			cat: auxcomLANGSPEC_Category;
     ...
     reltreelist1 = record
			first:auxcomDECL_prsnode;
			last:auxcomDECL_prsnode
		   end;
     sentencelength = 0..maximumlength;
     grammarlength = 0..auxcomRULES_maxrnr;
    
{*end earley types}

{*begin earley global variables:}
var la:auxcomDECL_psnode;
    i:integer;
    IT:array[0..maximumlength] of setofitems;
    ST:array[0..maximumlength] of setofsetofstrees;
    imax:sentencelength;
    topcats:auxcomDECL_syntcatset;
    initialitem:array[1..auxcomRULES_maxrnr] of pitem;
    libint:integer;
...
procedure newrsnode(var p:auxcomDECL_prsnode);
begin
   MEM_newdefault(loopholes.sizeof(auxcomDECL_rsnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlostree(var p:auxcomDECL_plostree);
begin
   MEM_newdefault(loopholes.sizeof(auxcomDECL_lostree),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlossurfnode(var p:auxcomDECL_plossurfgraphnode);
begin
   MEM_newdefault(loopholes.sizeof(auxcomDECL_lossurfgraphnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure taketree(var stree1: auxcomDECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:auxcomDECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1: auxcomDECL_plostree;
...
procedure appendnode(node1:auxcomDECL_psurfgraphnode;var S1:auxcomDECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:auxcomDECL_plossurfgraphnode;
...
procedure appendnodelist(S1:auxcomDECL_surfgraphnodelist;var S2:auxcomDECL_surfgraphnodelist);
...
procedure initnodelist(var S1:auxcomDECL_surfgraphnodelist);
...
function stillnodes(var S1:auxcomDECL_surfgraphnodelist):boolean;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:auxcomDECL_psurfgraphnode;var S1:auxcomDECL_surfgraphnodelist);
...
procedure copynodelist(var S1:auxcomDECL_surfgraphnodelist;var S2:auxcomDECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:auxcomDECL_surfgraphnodelist;
...
procedure takenodelist(var S1:auxcomDECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:auxcomDECL_psnode;var RT1:reltreelist1;
                        sonnumber:auxcomDECL_nodeid);
var prsnode1:auxcomDECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:auxcomDECL_psurfgraph;var node1:auxcomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:auxcomDECL_surfgraphnodelist;
                var nexttops1:auxcomDECL_syntcatset;var nextcats1:auxcomDECL_syntcatset);
...
var nextnodes2:auxcomDECL_surfgraphnodelist;
    result1,result2,result3,result4:auxcomDECL_syntcatset;
    node2:auxcomDECL_psurfgraphnode;
    cat1:auxcomLANGSPEC_Category;
    completed2:boolean;
...
procedure searchorread(graph1:auxcomDECL_psurfgraph;node2:auxcomDECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:auxcomDECL_surfgraphnodelist;
		var nexttops1:auxcomDECL_syntcatset;var nextcats1:auxcomDECL_syntcatset);
var node1:auxcomDECL_psurfgraphnode;
begin
...
with item1^ do
	begin
	cat:=auxcomGRAPHDEF_topcat(rnr1);
...
with item1^.ruleinfo do
	...
	searchorread(auxcomDECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:integer;node1:auxcomDECL_psurfgraphnode;
...
function local(item1:pitem;stree1:auxcomDECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:auxcomDECL_surfgraphnodelist;
    node1:auxcomDECL_psurfgraphnode;
    result:setofitems;
    rnr:integer;

function localsurfrule(item1:pitem;stree1:auxcomDECL_psnode;node1:auxcomDECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:auxcomDECL_nodeid;
    rnr,rnr2:integer;
    parents1:setofitems;
    rnrset:array[0..auxcomRULES_maxrnr] of boolean;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:auxcomDECL_surfgraphnodelist;

begin
auxcomDECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if auxcomDECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
auxcomRULES_rule(rnr,n,stree1,la,loccond);
if auxcomDECL_status then
	begin
	if auxcomDECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>auxcomRULES_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(auxcomDECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if auxcomDECL_debug then writeln('status false') end
end;

begin{local}
if auxcomDECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
if stree1^.cat in item1^.ruleinfo.nextcats then
	...
	rnr:=0;
	while rnr<>auxcomRULES_maxrnr do
		begin
		rnr:=rnr+1;
...
var 
    node1:auxcomDECL_psurfgraphnode;
    surfgraphnodelist1:auxcomDECL_surfgraphnodelist;
    result:setofstrees;
    stree1:auxcomDECL_psnode;
...
var S1:setofstrees;
    stree1,
    stree2 :auxcomDECL_psnode;
    Sij : setofstrees;

procedure saversnodes(var RT:reltreelist1);
var prsnode1,prsnode2,prsnode3:auxcomDECL_prsnode;

begin
prsnode1:=RT.first;
MEM_newdefault(loopholes.sizeof(auxcomDECL_rsnode),loopholes.retype(prsnode2,MEM_ptr));
prsnode2^:=prsnode1^;
RT.first:=prsnode2;
while prsnode1<>RT.last do
	...
	MEM_newdefault(loopholes.sizeof(auxcomDECL_rsnode),loopholes.retype(prsnode3,MEM_ptr));
        prsnode3^:=prsnode1^;
...
begin
if auxcomDECL_debug then writeln('->finalsurfrule');
auxcomDECL_assignstatus(false);
auxcomDECL_assignnewtop(nil);
inittreelist(S1);
readS(maxpos1, maxpos1+1, Sij);
taketree(stree2, Sij);
auxcomRULES_rule(rnr,Hfinal,nil,la,globcond);
if auxcomDECL_status then
	begin
	saversnodes(RT1);
	while auxcomDECL_newtop<>nil do
		begin
                newtop1:=auxcomDECL_newtop;
		...
		auxcomDECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:auxcomDECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:integer);
var stree1:auxcomDECL_psnode;
    node1:auxcomDECL_psurfgraphnode;
begin
if auxcomDECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if auxcomDECL_status then
		begin
		auxcomDECL_assignstatus(false);
		auxcomRULES_rule(rnr,node1^.n,stree1,la,globcond);
		nsons:=nsons+1;
...
begin{final}
if auxcomDECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		auxcomRULES_rule(rnr,Hinit,nil,la,globcond);
		nsons:=0;
		auxcomDECL_assignstatus(true);
		minpos:=item1^.minpos;
		maxpos:=item1^.curpos;
...
if stilltrees(result)=false then if auxcomDECL_debug then writeln('status false');
final:=result
end;
...
while rnr<>auxcomRULES_maxrnr do
	begin
	rnr:=rnr+1;
	if (auxcomGRAPHDEF_topcat(rnr) in item1^.ruleinfo.nexttops) then
		begin
...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if auxcomDECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
   ...
   {**}        IF i > (lasterror + 10) THEN
   			...
   			begin
                        ...
                        while w<>127 do
                           begin
                           w:=w+1;
                           IF auxcomLANGSPEC_scat(chr(w)) IN nextcats THEN
                              writeln('   ',chr(w))
                           end;
                        w:=0;
                        while w<>auxcomLANGSPEC_Nrow do
                           begin
                           w:=w+1;
                           IF auxcomLANGSPEC_wcat(w) IN nextcats THEN
                              BEGIN
                              auxcomLANGSPEC_word(w,str);
                              writeln('   ',str)
                              END
                           end;
...
var list1:auxcomDECL_psinglesegmlist;
    i,j:integer;
    stree1:auxcomDECL_psnode;
    connode1,connode2:auxcomDECL_pConnectionNode;
    Sij:setofstrees;
begin
...
if auxcomDECL_debug then writeln('imax determined:',imax); 
while i<>imax+1 do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if auxcomDECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}{ if auxcomDECL_debug then 
     		    begin
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if auxcomDECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}{ if auxcomDECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);auxcomdecl_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>auxcomRULES_maxrnr do
   begin
   i:=i+1;
...
{initialize topcats}
topcats := [];
FOR i:=1 TO auxcomRULES_maxrnr DO topcats := topcats + [auxcomGRAPHDEF_topcat(i)];
...
if auxcomDECL_debug then writeln('trees initialized');
initializeitems(imax);
if auxcomDECL_debug then writeln('items initialized');
...
while i <> imax do
	begin
	if auxcomDECL_debug then writeln(i);
