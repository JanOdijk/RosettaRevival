...
VAR
   newtopvar   : surfcomDECL_psinglesegmlist;
   statusvar   : boolean;
   debugvar    :  boolean;
   surfgraphvar: array[1..surfcomRULES_MAXRNR] OF surfcomDECL_psurfgraph;
   absentvar   : surfcomDECL_psurfgraph;
   countervar  : INTEGER;
   warningvar  : BOOLEAN;
...
FUNCTION surfcomDECL_newtop: surfcomDECL_psinglesegmlist;
BEGIN
   surfcomDECL_newtop:=newtopvar
END;

PROCEDURE surfcomDECL_assignnewtop(valeu: surfcomDECL_psinglesegmlist);
BEGIN
   newtopvar:=valeu
END;

PROCEDURE surfcomDECL_addnewtop(a: surfcomDECL_pSnode);
VAR
   c: surfcomDECL_psinglesegmlist;
BEGIN
   new(c);
   WITH c^ DO
      BEGIN
...
FUNCTION surfcomDECL_status:boolean;
BEGIN
   surfcomDECL_status:=statusvar
END;

PROCEDURE surfcomDECL_assignstatus(valeu:boolean);
BEGIN
   statusvar:=valeu
END;

FUNCTION surfcomDECL_debug:boolean;
BEGIN
   surfcomDECL_debug:=debugvar
END;

PROCEDURE surfcomDECL_assigndebug(valeu:boolean);
BEGIN
   debugvar:=valeu
END;

FUNCTION surfcomDECL_surfgraph(i:integer):surfcomDECL_psurfgraph;
BEGIN
   surfcomDECL_surfgraph:=surfgraphvar[i]
END;

PROCEDURE surfcomDECL_assignsurfgraph(i:integer; valeu: surfcomDECL_psurfgraph);
BEGIN
   surfgraphvar[i]:=valeu
END;

FUNCTION surfcomDECL_absent: surfcomDECL_psurfgraph;
BEGIN
   surfcomDECL_absent:=absentvar
END;

PROCEDURE surfcomDECL_assignabsent(valeu: surfcomDECL_psurfgraph);
BEGIN
   absentvar:=valeu
END;

PROCEDURE surfcomDECL_Addconnectionnode(VAR r: surfcomDECL_pconnectionnode;
                                                t: surfcomDECL_psnode); 
VAR p: surfcomDECL_pconnectionnode;
    l: surfcomDECL_psinglesegmlist;
...
FUNCTION surfcomDECL_MakeTerminal(s: surfcomLANGSPEC_Category;
                                          w: TSTRING_String): 
                                          surfcomDECL_psnode;
VAR
   t : surfcomDECL_psnode;
BEGIN
   ...
   t^.cat := s;
   t^.sonlist:=nil;
   surfcomLANGSPEC_CreateTerm(s, w, t^.ldfield);
   surfcomDECL_MakeTerminal := t
END;


FUNCTION surfcomDECL_MakeNonTerminal(s: surfcomLANGSPEC_Category):
                                          surfcomDECL_psnode;
VAR
   t : surfcomDECL_psnode;
BEGIN
   new(t);
   t^.cat := s;
   surfcomDECL_MakeNonTerminal := t
END;

FUNCTION surfcomDECL_Convinttostring(int:INTEGER):
                                          TSTRING_string;
...
surfcomDECL_Convinttostring:=result
END;
