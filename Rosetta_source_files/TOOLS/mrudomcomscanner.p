...
PROGRAM  MRUDOMCOMSCANNER;

CONST
   MAXLENGTH = 30;

FUNCTION MRUDOMCOMSCANNER_Body(VAR inf: text) : MRUDOMCOMDECL_pconnectionnode; 
...
VAR
   ...
   ch  : char;
   endoffile : boolean;
   sym : MRUDOMCOMLANGSPEC_Category;
   connected : boolean;
   wrd : TSTRING_String;
   cc,
   cl,
   ll  : integer;
   line: ARRAY[1..120] OF CHAR;
   t   : MRUDOMCOMDECL_pSnode;
   result: MRUDOMCOMDECL_pconnectionnode;
   ...
   BEGIN
      ...
      IF ch IN ['a'..'z','A'..'Z','0'..'9','''','¿'..'ý'] THEN 
         ...
         lower := 1; upper := MRUDOMCOMLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; MRUDOMCOMLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF ((lower-1 > upper) AND NOT(quote)) 
             THEN sym := MRUDOMCOMLANGSPEC_Wcat(k) 
             ELSE sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF ch = '{' THEN BEGIN
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := MRUDOMCOMDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         MRUDOMCOMDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   MRUDOMCOMSCANNER_body := result


END {MRUDOMCOMSCANNER_body};
