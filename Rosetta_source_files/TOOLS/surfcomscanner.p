...
FUNCTION surfcomSCANNER_Body(VAR inf: text) : surfcomDECL_pconnectionnode; 
...
VAR
   ...
   result: surfcomDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN surfcomLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in surfcomLANGSPEC_scanchars);
  
         lower := 1; upper := surfcomLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; surfcomLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := surfcomLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := surfcomLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := surfcomLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := surfcomDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         surfcomDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   surfcomSCANNER_body := result


END {surfcomSCANNER_body};
