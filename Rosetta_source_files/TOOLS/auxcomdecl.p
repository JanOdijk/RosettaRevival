...
VAR
   newtopvar   : auxcomDECL_psinglesegmlist;
   statusvar   : boolean;
   debugvar    :  boolean;
   surfgraphvar: array[1..auxcomRULES_MAXRNR] OF auxcomDECL_psurfgraph;
   absentvar   : auxcomDECL_psurfgraph;
   countervar  : INTEGER;
   warningvar  : BOOLEAN;
...
FUNCTION auxcomDECL_newtop: auxcomDECL_psinglesegmlist;
BEGIN
   auxcomDECL_newtop:=newtopvar
END;

PROCEDURE auxcomDECL_assignnewtop(valeu: auxcomDECL_psinglesegmlist);
BEGIN
   newtopvar:=valeu
END;

PROCEDURE auxcomDECL_addnewtop(a: auxcomDECL_pSnode);
VAR
   c: auxcomDECL_psinglesegmlist;
BEGIN
   new(c);
   WITH c^ DO
      BEGIN
...
FUNCTION auxcomDECL_status:boolean;
BEGIN
   auxcomDECL_status:=statusvar
END;

PROCEDURE auxcomDECL_assignstatus(valeu:boolean);
BEGIN
   statusvar:=valeu
END;

FUNCTION auxcomDECL_debug:boolean;
BEGIN
   auxcomDECL_debug:=debugvar
END;

PROCEDURE auxcomDECL_assigndebug(valeu:boolean);
BEGIN
   debugvar:=valeu
END;

FUNCTION auxcomDECL_surfgraph(i:integer):auxcomDECL_psurfgraph;
BEGIN
   auxcomDECL_surfgraph:=surfgraphvar[i]
END;

PROCEDURE auxcomDECL_assignsurfgraph(i:integer; valeu: auxcomDECL_psurfgraph);
BEGIN
   surfgraphvar[i]:=valeu
END;

FUNCTION auxcomDECL_absent: auxcomDECL_psurfgraph;
BEGIN
   auxcomDECL_absent:=absentvar
END;

PROCEDURE auxcomDECL_assignabsent(valeu: auxcomDECL_psurfgraph);
BEGIN
   absentvar:=valeu
END;

PROCEDURE auxcomDECL_Addconnectionnode(VAR r: auxcomDECL_pconnectionnode;
                                                t: auxcomDECL_psnode); 
VAR p: auxcomDECL_pconnectionnode;
    l: auxcomDECL_psinglesegmlist;
...
FUNCTION auxcomDECL_MakeTerminal(s: auxcomLANGSPEC_Category;
                                          w: TSTRING_String): 
                                          auxcomDECL_psnode;
VAR
   t : auxcomDECL_psnode;
BEGIN
   ...
   t^.cat := s;
   t^.sonlist:=nil;
   auxcomLANGSPEC_CreateTerm(s, w, t^.ldfield);
   auxcomDECL_MakeTerminal := t
END;


FUNCTION auxcomDECL_MakeNonTerminal(s: auxcomLANGSPEC_Category):
                                          auxcomDECL_psnode;
VAR
   t : auxcomDECL_psnode;
BEGIN
   new(t);
   t^.cat := s;
   auxcomDECL_MakeNonTerminal := t
END;

FUNCTION auxcomDECL_Convinttostring(int:INTEGER):
                                          TSTRING_string;
...
auxcomDECL_Convinttostring:=result
END;
