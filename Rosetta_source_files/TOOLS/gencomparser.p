...
PROCEDURE #PARSER_Body(if1 : #DECL_pConnectionNode; var if2 : #DECL_plostree);
{*earley constants:} 
...
type 
     ...
     rulerec = record
			...
			completed:boolean;
			nextnodes:#DECL_surfgraphnodelist;
			nexttops:#DECL_syntcatset;
			nextcats:#DECL_syntcatset
	       end;
     item = record
			cat: #LANGSPEC_Category;
...
var la:#DECL_psnode;
    ...
    imax:sentencelength;
    topcats:#DECL_syntcatset;
    initialitem:array[1..#RULES_maxrnr] of pitem;
    libint:integer;
...
procedure newrsnode(var p:#DECL_prsnode);
begin
   MEM_newdefault(loopholes.sizeof(#DECL_rsnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlostree(var p:#DECL_plostree);
begin
   MEM_newdefault(loopholes.sizeof(#DECL_lostree),loopholes.retype(p,MEM_ptr))
end;
...
procedure newlossurfnode(var p:#DECL_plossurfgraphnode);
begin
   MEM_newdefault(loopholes.sizeof(#DECL_lossurfgraphnode),loopholes.retype(p,MEM_ptr))
end;
...
procedure taketree(var stree1: #DECL_psnode;var S1:setofstrees);
...
procedure appendtree(stree1:#DECL_psnode;var S1:setofstrees);
{*appends stree1 at the end! of S1, if stree1 <> nil,
 *else S1 is not changed.}
var plostree1: #DECL_plostree;
...
procedure appendnode(node1:#DECL_psurfgraphnode;var S1:#DECL_surfgraphnodelist);
{*puts node1 at the end! of S1, if node1 <> nil.}
var plossurfgraphnode1:#DECL_plossurfgraphnode;
...
procedure appendnodelist(S1:#DECL_surfgraphnodelist;var S2:#DECL_surfgraphnodelist);
...
procedure initnodelist(var S1:#DECL_surfgraphnodelist);
...
function stillnodes(var S1:#DECL_surfgraphnodelist):boolean;
{*is true iff S1 is empty, i.e. if not S1.first=nil}
begin
...
procedure takenode(var node1:#DECL_psurfgraphnode;var S1:#DECL_surfgraphnodelist);
...
procedure copynodelist(var S1:#DECL_surfgraphnodelist;var S2:#DECL_surfgraphnodelist);
...
procedure appendsurfnodelist(var S1:#DECL_surfgraphnodelist;
...
procedure takenodelist(var S1:#DECL_surfgraphnodelist;var L1:listofsurfnodelists);
begin
...
procedure appendreltree(stree1:#DECL_psnode;var RT1:reltreelist1;
                        sonnumber:#DECL_nodeid);
var prsnode1:#DECL_prsnode;
begin
newrsnode(prsnode1);
...
procedure searchgraph(graph1:#DECL_psurfgraph;var node1:#DECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:#DECL_surfgraphnodelist;
                var nexttops1:#DECL_syntcatset;var nextcats1:#DECL_syntcatset);
...
var nextnodes2:#DECL_surfgraphnodelist;
    result1,result2,result3,result4:#DECL_syntcatset;
    node2:#DECL_psurfgraphnode;
    cat1:#LANGSPEC_Category;
    completed2:boolean;
...
procedure searchorread(graph1:#DECL_psurfgraph;node2:#DECL_psurfgraphnode;
		var completed1:boolean;var nextnodes1:#DECL_surfgraphnodelist;
		var nexttops1:#DECL_syntcatset;var nextcats1:#DECL_syntcatset);
var node1:#DECL_psurfgraphnode;
begin
...
with item1^.ruleinfo do
	...
	searchorread(#DECL_surfgraph(rnr),nil,completed,
...
function sameitem(var item1:pitem;minpos:integer;node1:#DECL_psurfgraphnode;
...
function local(item1:pitem;stree1:#DECL_psnode;IT:setofitems):setofitems;
...
var nextnodes1:#DECL_surfgraphnodelist;
    node1:#DECL_psurfgraphnode;
    result:setofitems;
    rnr:integer;

function localsurfrule(item1:pitem;stree1:#DECL_psnode;node1:#DECL_psurfgraphnode):pitem;
var item2,item3,item4:pitem;
    n:#DECL_nodeid;
    rnr,rnr2:integer;
    parents1:setofitems;
    rnrset:array[0..#RULES_maxrnr] of boolean;
    adoptedsons1,adoptedsons2:setofsetofstrees;
    setofstrees1,setofstrees2:setofstrees;
    history1,history2:listofsurfnodelists;
    surfgraphnodelist1,surfgraphnodelist2:#DECL_surfgraphnodelist;

begin
#DECL_assignstatus(false);
n:=node1^.n;
rnr:=item1^.ruleinfo.rnr;
if #DECL_debug then write('->localsurfrule',rnr,'  idnr:',n);
#RULES_rule(rnr,n,stree1,la,loccond);
if #DECL_status then
	begin
	if #DECL_debug then writeln('status true');
	if item1^.minpos=item1^.curpos then
		{*mark parents of initial items}
		begin
		rnr2:=0;
		while rnr2<>#RULES_maxrnr do
			begin
			rnr2:=rnr2+1;
			rnrset[rnr2]:=false
			end;
	...
	if sameitem(item4,item1^.minpos,node1,IT)=false then
		...
		with item2^.ruleinfo do 
			...
			history:=history2;
			initnodelist(nextnodes);
			searchorread(#DECL_surfgraph(rnr),node1,
...
else begin localsurfrule:=nil; if #DECL_debug then writeln('status false') end
end;

begin{local}
if #DECL_debug then writeln('->local',item1^.ruleinfo.rnr,'  minpos:',item1^.minpos);
inititemlist(result);
...
var 
    node1:#DECL_psurfgraphnode;
    surfgraphnodelist1:#DECL_surfgraphnodelist;
    result:setofstrees;
    stree1:#DECL_psnode;
...
var prsnode1,prsnode2,prsnode3:#DECL_prsnode;

begin
prsnode1:=RT.first;
MEM_newdefault(loopholes.sizeof(#DECL_rsnode),loopholes.retype(prsnode2,MEM_ptr));
prsnode2^:=prsnode1^;
RT.first:=prsnode2;
while prsnode1<>RT.last do
	...
	MEM_newdefault(loopholes.sizeof(#DECL_rsnode),loopholes.retype(prsnode3,MEM_ptr));
        prsnode3^:=prsnode1^;
...
begin
if #DECL_debug then writeln('->finalsurfrule');
#DECL_assignstatus(false);
#DECL_assignnewtop(nil);
inittreelist(S1);
readS(maxpos1, maxpos1+1, Sij);
taketree(stree2, Sij);
#RULES_rule(rnr,Hfinal,nil,la,globcond);
if #DECL_status then
	begin
	saversnodes(RT1);
	while #DECL_newtop<>nil do
		begin
                newtop1:=#DECL_newtop;
		...
		#DECL_assignnewtop(newtop1^.next)
		end
	end;
finalsurfrule:=S1
end;
...
procedure globalsurfrule(var setofstrees1:setofstrees;
		var surfgraphnodelist1:#DECL_surfgraphnodelist;var RT1:reltreelist1;
							    var nsons:integer);
var stree1:#DECL_psnode;
    node1:#DECL_psurfgraphnode;
begin
if #DECL_debug then writeln('->globalsurfrule');
while stilltrees(setofstrees1) do
	...
	if #DECL_status then
		begin
		#DECL_assignstatus(false);
		#RULES_rule(rnr,node1^.n,stree1,la,globcond);
		nsons:=nsons+1;
...
begin{final}
if #DECL_debug then write('->final',item1^.ruleinfo.rnr);
inittreelist(result);
if item1^.ruleinfo.completed then
	...
	while stilltreelists(adoptedsons1) do
		...
		rnr:=item1^.ruleinfo.rnr;
		#RULES_rule(rnr,Hinit,nil,la,globcond);
		nsons:=0;
		#DECL_assignstatus(true);
		minpos:=item1^.minpos;
		maxpos:=item1^.curpos;
...
if stilltrees(result)=false then if #DECL_debug then writeln('status false');
final:=result
end;
...
while rnr<>#RULES_maxrnr do
	begin
	rnr:=rnr+1;
	if (#GRAPHDEF_topcat(rnr) in item1^.ruleinfo.nexttops) then
		begin
...
while stilltreelists(setofSij) do
	...
	while stilltrees(S1) do
		...
		if #DECL_debug then writeln('tree scanned');
		copyitemlist(I1,IT[i]);
   ...
   {**}        IF i > (lasterror + 10) THEN
   			...
   			begin
                        ...
                        while w<>127 do
                           begin
                           w:=w+1;
                           IF #LANGSPEC_scat(chr(w)) IN nextcats THEN
                              writeln('   ',chr(w))
                           end;
                        w:=0;
                        while w<>#LANGSPEC_Nrow do
                           begin
                           w:=w+1;
                           IF #LANGSPEC_wcat(w) IN nextcats THEN
                              BEGIN
                              #LANGSPEC_word(w,str);
                              writeln('   ',str)
                              END
...
var list1:#DECL_psinglesegmlist;
    i,j:integer;
    stree1:#DECL_psnode;
    connode1,connode2:#DECL_pConnectionNode;
    Sij:setofstrees;
begin
...
if #DECL_debug then writeln('imax determined:',imax); 
while i<>imax+1 do
	begin
	{j:=i;
	while j<>imax do
   ...
   while connode1<>connode2 do
	...
	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=j;
                if #DECL_debug then writeln('input i,j:',i,j,'cat:',ord(stree1^.cat));
   		{*}{ if #DECL_debug then 
     		    begin
	...
	connode1:=connode1^.next
   	...
   	while list1<>nil do
		...
		stree1^.minpos:=i;stree1^.maxpos:=imax;
                if #DECL_debug then writeln('input i,j:',i,imax,'cat:',ord(stree1^.cat));
   		{*}{ if #DECL_debug then 
   		    begin
   	            if stree1^.cat=charstring then 
   		    writeln('charstring=',stree1^.charstringfield^.stringfield);
...
new(absent1);#decl_assignabsent(absent1);
i:=0;
usedparentlistindices:=[];
while i<>#RULES_maxrnr do
   begin
   i:=i+1;
...
{initialize topcats}
topcats := [];
FOR i:=1 TO #RULES_maxrnr DO topcats := topcats + [#GRAPHDEF_topcat(i)];
...
if #DECL_debug then writeln('trees initialized');
initializeitems(imax);
if #DECL_debug then writeln('items initialized');
