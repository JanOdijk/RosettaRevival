...
VAR
   newtopvar   : mrucomDECL_psinglesegmlist;
   statusvar   : boolean;
   debugvar    :  boolean;
   surfgraphvar: array[1..mrucomRULES_MAXRNR] OF mrucomDECL_psurfgraph;
   absentvar   : mrucomDECL_psurfgraph;
   countervar  : INTEGER;
   warningvar  : BOOLEAN;
...
FUNCTION mrucomDECL_newtop: mrucomDECL_psinglesegmlist;
BEGIN
   mrucomDECL_newtop:=newtopvar
END;

PROCEDURE mrucomDECL_assignnewtop(valeu: mrucomDECL_psinglesegmlist);
BEGIN
   newtopvar:=valeu
END;

PROCEDURE mrucomDECL_addnewtop(a: mrucomDECL_pSnode);
VAR
   c: mrucomDECL_psinglesegmlist;
BEGIN
   new(c);
   WITH c^ DO
      BEGIN
...
FUNCTION mrucomDECL_status:boolean;
BEGIN
   mrucomDECL_status:=statusvar
END;

PROCEDURE mrucomDECL_assignstatus(valeu:boolean);
BEGIN
   statusvar:=valeu
END;

FUNCTION mrucomDECL_debug:boolean;
BEGIN
   mrucomDECL_debug:=debugvar
END;

PROCEDURE mrucomDECL_assigndebug(valeu:boolean);
BEGIN
   debugvar:=valeu
END;

FUNCTION mrucomDECL_surfgraph(i:integer):mrucomDECL_psurfgraph;
BEGIN
   mrucomDECL_surfgraph:=surfgraphvar[i]
END;

PROCEDURE mrucomDECL_assignsurfgraph(i:integer; valeu: mrucomDECL_psurfgraph);
BEGIN
   surfgraphvar[i]:=valeu
END;

FUNCTION mrucomDECL_absent: mrucomDECL_psurfgraph;
BEGIN
   mrucomDECL_absent:=absentvar
END;

PROCEDURE mrucomDECL_assignabsent(valeu: mrucomDECL_psurfgraph);
BEGIN
   absentvar:=valeu
END;

PROCEDURE mrucomDECL_Addconnectionnode(VAR r: mrucomDECL_pconnectionnode;
                                                t: mrucomDECL_psnode); 
VAR p: mrucomDECL_pconnectionnode;
    l: mrucomDECL_psinglesegmlist;
...
FUNCTION mrucomDECL_MakeTerminal(s: mrucomLANGSPEC_Category;
                                          w: TSTRING_String): 
                                          mrucomDECL_psnode;
VAR
   t : mrucomDECL_psnode;
BEGIN
   ...
   t^.cat := s;
   t^.sonlist:=nil;
   mrucomLANGSPEC_CreateTerm(s, w, t^.ldfield);
   mrucomDECL_MakeTerminal := t
END;


FUNCTION mrucomDECL_MakeNonTerminal(s: mrucomLANGSPEC_Category):
                                          mrucomDECL_psnode;
VAR
   t : mrucomDECL_psnode;
BEGIN
   new(t);
   t^.cat := s;
   mrucomDECL_MakeNonTerminal := t
END;

FUNCTION mrucomDECL_Convinttostring(int:INTEGER):
                                          TSTRING_string;
...
mrucomDECL_Convinttostring:=result
END;
