...
FUNCTION afxprSCANNER_Body(VAR inf: text) : afxprDECL_pconnectionnode; 
...
VAR
   ...
   t   : afxprDECL_pSnode;
   result: afxprDECL_pconnectionnode;
   ...
   WHILE ch = ' ' DO 
         ...
         getch;
         END;
   IF ch IN afxprLANGSPEC_scanchars THEN BEGIN
         ...
         REPEAT
            IF k < MAXLENGTH THEN 
               ...
               END;
            getch
         UNTIL NOT (ch in afxprLANGSPEC_scanchars);
  
         lower := 1; upper := afxprLANGSPEC_Nrow;
         REPEAT
            k := (lower+upper) DIV 2; afxprLANGSPEC_Word(k, w);
            IF wrd <= w THEN upper := k-1;
            IF wrd >= w THEN lower := k+1;
         UNTIL lower > upper;
         IF lower-1 > upper THEN sym := afxprLANGSPEC_Wcat(k) ELSE 
                                                        sym := IDENTIFIER;
         END
      ELSE BEGIN
         IF false {..} THEN BEGIN
         ...
         ELSE 
   		...
   		BEGIN
   		    ...
   		    BEGIN
   	            ...
   	            END
                ELSE BEGIN
                     sym := afxprLANGSPEC_SCAT('('); 
                     TSTRING_ConstantChar(wrd,'(')
                     END
   		END
            ELSE IF ch<>'!' THEN BEGIN
                 sym := afxprLANGSPEC_SCAT(ch); 
...
BEGIN
   ...
   REPEAT
      ...
      IF sym <> COMMENT THEN BEGIN
         t := afxprDECL_maketerminal(sym, wrd);
         t^.cl := cl;
         t^.connected := connected;
         afxprDECL_Addconnectionnode(result, t)
      END
   UNTIL endoffile;

99:
   afxprSCANNER_body := result


END {afxprSCANNER_body};
